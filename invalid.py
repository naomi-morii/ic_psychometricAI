...
csv_folder = 'nocontext'

# Dicionário para armazenar os resultados
results = {}

# Gera uma lista de arquivos ordenados pelo número no nome
csv_files = [f'bfi_responses_nocontext{num}.csv' for num in range(1, 51)]
...
# Ordena o dicionário por chave (que já são os IDs numéricos no formato bfi_response_incontextX)
sorted_results = dict(sorted(results.items(), key=lambda item: int(item[0].split('nocontext')[-1])))

# Salva os resultados ordenados em um arquivo JSON
